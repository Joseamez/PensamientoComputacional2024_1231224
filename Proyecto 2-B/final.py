...
def configurar_zonas():
    while True:
        opcion = input("¿Desea agregar una nueva zona (A), eliminar una zona existente (E) o salir (S)? ").upper()
        if opcion == "A":
...
def programar_horarios():
    ...
    if zona_seleccionada in zonas:
        opcion = input("¿Desea programar un nuevo horario (N) o eliminar un horario existente (E)? ").upper()
        if opcion == "N":
